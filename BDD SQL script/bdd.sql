-- MySQL Script generated by MySQL Workbench
-- Tue May  2 17:08:57 2017
-- Model: New Model    Version: 1.0
-- MySQL Workbench Forward Engineering

SET @OLD_UNIQUE_CHECKS=@@UNIQUE_CHECKS, UNIQUE_CHECKS=0;
SET @OLD_FOREIGN_KEY_CHECKS=@@FOREIGN_KEY_CHECKS, FOREIGN_KEY_CHECKS=0;
SET @OLD_SQL_MODE=@@SQL_MODE, SQL_MODE='TRADITIONAL,ALLOW_INVALID_DATES';

-- -----------------------------------------------------
-- Schema academia
-- -----------------------------------------------------

-- -----------------------------------------------------
-- Schema academia
-- -----------------------------------------------------
CREATE SCHEMA IF NOT EXISTS `academia` DEFAULT CHARACTER SET utf8 ;
USE `academia` ;

-- -----------------------------------------------------
-- Table `academia`.`Person`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `academia`.`Person` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `dateYear` VARCHAR(45) NULL,
  `login` VARCHAR(45) NULL,
  `typePerson` VARCHAR(45) NULL,
  `speciality` VARCHAR(45) NULL,
  `name` VARCHAR(45) NULL,
  PRIMARY KEY (`id`))
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `academia`.`Subject`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `academia`.`Subject` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `title` VARCHAR(45) NULL,
  `content` TEXT NULL,
  PRIMARY KEY (`id`))
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `academia`.`Team`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `academia`.`Team` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `Person_id` INT NULL,
  `Subject_id` INT NULL,
  `name` VARCHAR(45) NULL,
  PRIMARY KEY (`id`),
  INDEX `fk_Team_Person_idx` (`Person_id` ASC),
  INDEX `fk_Team_Subject1_idx` (`Subject_id` ASC),
  CONSTRAINT `fk_Team_Person`
    FOREIGN KEY (`Person_id`)
    REFERENCES `academia`.`Person` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_Team_Subject1`
    FOREIGN KEY (`Subject_id`)
    REFERENCES `academia`.`Subject` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `academia`.`NoteCard`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `academia`.`NoteCard` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `Team_id` INT NULL,
  `Person_id` INT NULL,
  PRIMARY KEY (`id`),
  INDEX `fk_NoteCard_Team1_idx` (`Team_id` ASC),
  INDEX `fk_NoteCard_Person1_idx` (`Person_id` ASC),
  CONSTRAINT `fk_NoteCard_Team1`
    FOREIGN KEY (`Team_id`)
    REFERENCES `academia`.`Team` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_NoteCard_Person1`
    FOREIGN KEY (`Person_id`)
    REFERENCES `academia`.`Person` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `academia`.`Feature`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `academia`.`Feature` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `content` TEXT NULL,
  `state` TINYINT NULL,
  `typePerson` VARCHAR(45) NULL,
  `name` VARCHAR(45) NULL,
  `Subject_id` INT NULL,
  `Team_id` INT NULL,
  PRIMARY KEY (`id`),
  INDEX `fk_Feature_Subject1_idx` (`Subject_id` ASC),
  INDEX `fk_Feature_Team1_idx` (`Team_id` ASC),
  CONSTRAINT `fk_Feature_Subject1`
    FOREIGN KEY (`Subject_id`)
    REFERENCES `academia`.`Subject` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_Feature_Team1`
    FOREIGN KEY (`Team_id`)
    REFERENCES `academia`.`Team` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `academia`.`Comment`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `academia`.`Comment` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `content` TEXT NULL,
  `author` VARCHAR(45) NULL,
  PRIMARY KEY (`id`))
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `academia`.`Appointment`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `academia`.`Appointment` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `dateA` VARCHAR(45) NULL,
  `hStart` VARCHAR(45) NULL,
  `hEnd` VARCHAR(45) NULL,
  `content` TEXT NULL,
  `NoteCard_id` INT NULL,
  `Person_id` INT NULL,
  `Comment_id` INT NULL,
  `Feature_id` INT NULL,
  PRIMARY KEY (`id`),
  INDEX `fk_Appointment_NoteCard1_idx` (`NoteCard_id` ASC),
  INDEX `fk_Appointment_Person1_idx` (`Person_id` ASC),
  INDEX `fk_Appointment_Comment1_idx` (`Comment_id` ASC),
  INDEX `fk_Appointment_Feature1_idx` (`Feature_id` ASC),
  CONSTRAINT `fk_Appointment_NoteCard1`
    FOREIGN KEY (`NoteCard_id`)
    REFERENCES `academia`.`NoteCard` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_Appointment_Person1`
    FOREIGN KEY (`Person_id`)
    REFERENCES `academia`.`Person` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_Appointment_Comment1`
    FOREIGN KEY (`Comment_id`)
    REFERENCES `academia`.`Comment` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_Appointment_Feature1`
    FOREIGN KEY (`Feature_id`)
    REFERENCES `academia`.`Feature` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `academia`.`Document`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `academia`.`Document` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `depoDate` VARCHAR(45) NULL,
  `path` VARCHAR(45) NULL,
  `Team_id` INT NULL,
  PRIMARY KEY (`id`),
  INDEX `fk_Document_Team1_idx` (`Team_id` ASC),
  CONSTRAINT `fk_Document_Team1`
    FOREIGN KEY (`Team_id`)
    REFERENCES `academia`.`Team` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `academia`.`DepositBox`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `academia`.`DepositBox` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `limitDate` VARCHAR(45) NULL,
  `title` VARCHAR(45) NULL,
  `Document_id` INT NULL,
  PRIMARY KEY (`id`),
  INDEX `fk_DepositBox_Document1_idx` (`Document_id` ASC),
  CONSTRAINT `fk_DepositBox_Document1`
    FOREIGN KEY (`Document_id`)
    REFERENCES `academia`.`Document` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `academia`.`wallMessage`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `academia`.`wallMessage` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `content` TEXT NULL,
  `Person_id` INT NULL,
  PRIMARY KEY (`id`),
  INDEX `fk_wallMessage_Person1_idx` (`Person_id` ASC),
  CONSTRAINT `fk_wallMessage_Person1`
    FOREIGN KEY (`Person_id`)
    REFERENCES `academia`.`Person` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `academia`.`wallCom`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `academia`.`wallCom` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `content` TEXT NULL,
  `wallMessage_id` INT NULL,
  PRIMARY KEY (`id`),
  INDEX `fk_wallCom_wallMessage1_idx` (`wallMessage_id` ASC),
  CONSTRAINT `fk_wallCom_wallMessage1`
    FOREIGN KEY (`wallMessage_id`)
    REFERENCES `academia`.`wallMessage` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `academia`.`Feature_has_NoteCard`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `academia`.`Feature_has_NoteCard` (
  `Feature_id` INT NULL,
  `NoteCard_id` INT NULL,
  `id` INT NOT NULL AUTO_INCREMENT,
  INDEX `fk_Feature_has_NoteCard_NoteCard1_idx` (`NoteCard_id` ASC),
  INDEX `fk_Feature_has_NoteCard_Feature1_idx` (`Feature_id` ASC),
  PRIMARY KEY (`id`),
  CONSTRAINT `fk_Feature_has_NoteCard_Feature1`
    FOREIGN KEY (`Feature_id`)
    REFERENCES `academia`.`Feature` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_Feature_has_NoteCard_NoteCard1`
    FOREIGN KEY (`NoteCard_id`)
    REFERENCES `academia`.`NoteCard` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


SET SQL_MODE=@OLD_SQL_MODE;
SET FOREIGN_KEY_CHECKS=@OLD_FOREIGN_KEY_CHECKS;
SET UNIQUE_CHECKS=@OLD_UNIQUE_CHECKS;
